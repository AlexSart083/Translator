...
class MainWindow(QMainWindow):
    ...
    def _build_toolbar(self) -> QWidget:
        ...
        # Language controls (hidden when not in Translate mode)
        self._lang_widgets = []

        def track_lang(widget):
            self._lang_widgets.append(widget)
            layout.addWidget(widget)
            return widget

        src_label = QLabel("From")
        src_label.setObjectName("toolbarLabel")
        track_lang(src_label)

        self.source_lang_combo = QComboBox()
        self.source_lang_combo.addItems(LANGUAGE_NAMES)
        self.source_lang_combo.setFixedWidth(150)
        track_lang(self.source_lang_combo)
        ...
        track_lang(self.target_lang_combo)
    ...
    def _build_source_panel(self) -> QWidget:
        ...
        self.source_title_label = QLabel("Source Text")
        self.source_title_label.setObjectName("panelTitle")
        hlayout.addWidget(self.source_title_label)
        hlayout.addStretch()
    ...
    def _build_target_panel(self) -> QWidget:
        ...
        self.target_title_label = QLabel("Translation")
        self.target_title_label.setObjectName("panelTitle")
        hlayout.addWidget(self.target_title_label)
        hlayout.addStretch()
    ...
    def _on_mode_changed(self, mode: str):
        """Show/hide language controls and update UI labels based on mode."""
        is_translate = mode == "Translate"
        for widget in self._lang_widgets:
            widget.setVisible(is_translate)

        # Update panel titles and placeholders
        if mode == "Translate":
            self.source_title_label.setText("Source Text")
            self.target_title_label.setText("Translation")
            self.source_edit.setPlaceholderText(
                "Enter text to translate…\n\n"
                "Tip: press Ctrl+Enter to translate quickly."
            )
            ...
            self.source_title_label.setText("Original Text")
            self.target_title_label.setText("Summary")
            self.source_edit.setPlaceholderText(
                "Enter text to summarize…\n\n"
                "Tip: press Ctrl+Enter to summarize quickly."
            )
            self.target_edit.setPlaceholderText("Summary will appear here…")
            self.translate_btn.setText("  Summarize  ")
        elif mode == "Improve":
            self.source_title_label.setText("Original Text")
            self.target_title_label.setText("Improved Text")
            self.source_edit.setPlaceholderText(
                "Enter text to improve…\n\n"
                "Tip: press Ctrl+Enter to improve quickly."
            )
            self.target_edit.setPlaceholderText("Improved text will appear here…")
            self.translate_btn.setText("  Improve  ")
    ...
    def _start_translation(self):
        ...
        if not text:
            ...
            action = mode.lower()
            self._set_status(f"Enter some text to {action}.", "info")
            return
        ...
        def _worker():
            try:
                if mode == "Translate":
                    ...
                    result = translator.summarize(text)
                elif mode == "Improve":
                    result = translator.improve(text)
                else:
                    raise TranslationError(f"Unknown mode: {mode}")
    ...
    def _set_busy(self, busy: bool, mode: str = "Translate"):
        ...
        if busy:
            if mode == "Translate":
                ...
                label = "  Processing…  "
                status = "Processing…"
            self.translate_btn.setText(label)
    ...
    def _on_translation_done(self, result: str):
        ...
        if mode == "Translate":
            ...
            self._set_status("Summary complete.", "ok")
        elif mode == "Improve":
            self._set_status("Improvement complete.", "ok")
        save_settings(self.settings)
    ...
    def _on_translation_error(self, message: str):
        ...
        self.target_edit.setPlainText(f"⚠  Operation failed\n\n{message}")
    ...
    def _restore_settings(self):
        ...
        # Restore mode
        mode = self.settings.get("mode", "Translate")
        mode_idx = self.mode_combo.findText(mode)
        if mode_idx >= 0:
            self.mode_combo.setCurrentIndex(mode_idx)
        # Apply mode visibility immediately
